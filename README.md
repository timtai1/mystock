...
- **多組自選股**:左側選單支援任意多份自選股清單,每份對應一個 `stocklist_{群組名}.txt` 檔,可在頁面上直接新增 / 改名 / 刪除。
  ...
  - `fetch_target_price.py` 與 `fetch_daily_kline.py` 都會自動掃描所有 `stocklist_*.txt`,跨清單去重後執行,確保資料完整性。
- **自動補回資料空窗**:若多日未執行,`fetch_daily_kline.py` 會自動檢測並補回漏掉的交易日(上市走 TWSE,上櫃走 FinMind),確保時間序列完整。
